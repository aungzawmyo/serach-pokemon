...
import { Pokemon, BasePokemon, PokemonsData } from "../types/pokemon";
...
export default function PokemonListClient({ initialData }: PokemonListClientProps) { 
  const [searchName, setSearchName] = useState("");
  const router = useRouter();

  // Use Apollo Query with fallback to initialData
  ...
  // Use static data if available, otherwise use Apollo data
  const pokemonData = initialData || data?.pokemons || [];
  ...
  if (isLoading) return (
    <div className="container mx-auto p-4">
      <div className="flex justify-center items-center min-h-[400px]">
        <div className="text-xl animate-pulse">Loading Pokemon...</div>
      </div>
    </div>
  );
      ...
      <div>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-bold">
            ...
            {initialData && <span className="text-sm text-green-600 ml-2">(  Statically Optimized)</span>}
          </h2>
        </div>
